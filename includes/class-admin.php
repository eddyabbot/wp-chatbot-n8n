			...
			<form method="post" action="">
				...
				<div id="wpcn-panel-general" class="wpcn-tab-panel">
					<table class="form-table" role="presentation">
						...
						<tr>
							<th scope="row"><label for="wpcn-mode"><?php esc_html_e( 'Modalità widget', 'wp-chatbot-n8n' ); ?></label></th>
							<td>
								<select id="wpcn-mode" name="mode">
									<option value="window" <?php selected( $s['mode'], 'window' ); ?>><?php esc_html_e( 'Finestra flottante', 'wp-chatbot-n8n' ); ?></option>
									<option value="fullscreen" <?php selected( $s['mode'], 'fullscreen' ); ?>><?php esc_html_e( 'Schermo intero', 'wp-chatbot-n8n' ); ?></option>
								</select>
							</td>
						</tr>

						<tr>
							<th scope="row"><label for="wpcn-position"><?php esc_html_e( 'Posizione widget', 'wp-chatbot-n8n' ); ?></label></th>
							<td>
								<select id="wpcn-position" name="position">
									<option value="bottom-right" <?php selected( $s['position'], 'bottom-right' ); ?>><?php esc_html_e( 'In basso a destra', 'wp-chatbot-n8n' ); ?></option>
									<option value="bottom-left" <?php selected( $s['position'], 'bottom-left' ); ?>><?php esc_html_e( 'In basso a sinistra', 'wp-chatbot-n8n' ); ?></option>
								</select>
							</td>
						</tr>

						<tr>
							<th scope="row"><?php esc_html_e( 'Opzioni sessione', 'wp-chatbot-n8n' ); ?></th>
							<td>
								<fieldset>
									<label>
										<input type="checkbox" name="load_previous_session" value="1" <?php checked( $s['load_previous_session'], '1' ); ?> />
										<?php esc_html_e( 'Carica sessione precedente', 'wp-chatbot-n8n' ); ?>
									</label><br />
									<label>
										<input type="checkbox" name="show_welcome_screen" value="1" <?php checked( $s['show_welcome_screen'], '1' ); ?> />
										<?php esc_html_e( 'Mostra schermata di benvenuto', 'wp-chatbot-n8n' ); ?>
									</label><br />
									<label>
										<input type="checkbox" name="enable_streaming" value="1" <?php checked( $s['enable_streaming'], '1' ); ?> />
										<?php esc_html_e( 'Abilita streaming risposte', 'wp-chatbot-n8n' ); ?>
										<span class="description">&nbsp;<?php esc_html_e( '(richiede supporto nel workflow n8n)', 'wp-chatbot-n8n' ); ?></span>
									</label>
								</fieldset>
							</td>
						</tr>
						...
						<tr>
							<th scope="row"><?php esc_html_e( 'Upload file', 'wp-chatbot-n8n' ); ?></th>
							<td>
								<fieldset>
									<label>
										<input type="checkbox" id="wpcn-allow-file-uploads" name="allow_file_uploads" value="1" <?php checked( $s['allow_file_uploads'], '1' ); ?> />
										<?php esc_html_e( 'Consenti upload file in chat', 'wp-chatbot-n8n' ); ?>
									</label>
								</fieldset>
							</td>
						</tr>
						...
						<tr>
							<th scope="row"><label for="wpcn-allowed-mime-types"><?php esc_html_e( 'Tipi di file consentiti', 'wp-chatbot-n8n' ); ?></label></th>
							<td>
								<input type="text" id="wpcn-allowed-mime-types" name="allowed_mime_types"
									value="<?php echo esc_attr( $s['allowed_mime_types'] ); ?>"
									class="regular-text" placeholder="image/*,application/pdf" />
								<p class="description">
									<?php esc_html_e( 'MIME types separati da virgola. Lascia vuoto per consentire tutti i formati. Attivo solo se "Consenti upload file" è abilitato.', 'wp-chatbot-n8n' ); ?>
								</p>
							</td>
						</tr>
